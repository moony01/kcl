...
import { KPOPFACE_REPORT_PACK } from '../_shared/kpopface-payment.ts';
...
async function createCheckoutSession(params: {
  ...
  body.set('metadata[product]', KPOPFACE_REPORT_PACK.product);
  body.set('metadata[quantity]', String(KPOPFACE_REPORT_PACK.quantity));
  body.set('line_items[0][quantity]', '1');
  body.set('line_items[0][price_data][currency]', KPOPFACE_REPORT_PACK.currency);
  body.set('line_items[0][price_data][unit_amount]', String(KPOPFACE_REPORT_PACK.amountCents));
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { error: paymentInsertError } = await supabase.from('kpopface_payments').insert({
      ...
      quantity: KPOPFACE_REPORT_PACK.quantity,
      amount_cents: KPOPFACE_REPORT_PACK.amountCents,
